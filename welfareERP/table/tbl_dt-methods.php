                ...
                <div class="d-flex align-items-center">
                  ...
                  <div style="text-align: right;">
                    <select class="form-select" style="width: auto; padding: 0.5rem 2rem 0.5rem 0.5rem; border-radius: 5px; border: 1px solid #ced4da;" onchange="window.location.href=this.value">
                      <option value="../table/tbl_dt-methods.php" selected>현재지원</option>
                      <option value="../table/tbl_dt-methods1.php">거절지원</option>
                      <option value="../table/tbl_dt-methods2.php">수락지원</option>
                    </select>
                  </div>
                </div>
              </div>
...
and a.application_id = asd.application_id and asd.state = 'ready'"; // your_table_name은 데이터를 가져올 테이블 이름
  ...
  <script>
    function init() {
      ...
      $('#destroy').click(function() {
        ...
        if (selectedNames.length > 0) {
          var reason = prompt("삭제 이유를 입력하세요:");
          if (reason != null && reason.trim() != "") {
            alert(selectedNames.join(', ') + " 삭제 완료. 이유: " + reason);
            $.post('../dashboard/php/destroy.php', {selectedNames: selectedNames, reason: reason});
          } else {
            alert("삭제 이유를 입력하지 않았습니다. 삭제가 취소되었습니다.");
          }
        } else {
          alert("선택된 행이 없습니다.");
        }
